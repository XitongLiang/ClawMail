...
def generate_email(
    subject: str, outline: str, account_id: str
) -> str:
    """生成新邮件，返回纯文本正文。语气风格由 LLM 根据记忆自动判断。"""
    logger.info("生成新邮件: subject=%s", subject)
...
def main():
    ...
    try:
        result = generate_email(args.subject, args.outline, args.account_id)
        print(result)
