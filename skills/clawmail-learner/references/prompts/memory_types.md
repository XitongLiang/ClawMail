...
memory_key: signal 内容的关键词短语（如 "账单到期催缴"、"营销游戏推荐"、"项目调动"）
...
- memory_key 使用 preference_type 值，便于查重和更新

输出格式：
memory_type: summary_preference
memory_key: preference_type 值（"keywords" / "one_line" / "brief" / "general"）
...
memory_key: 收件人邮箱（针对性偏好）或 场景关键词如 "通用语气" "通用长度"（通用偏好）
